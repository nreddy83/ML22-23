...
#establishing centroids
kmeans = []
bestR = 0, 0
for i, a in enumerate(attributes):
    bestRange, bestAbt = bestR
    mx = 0
    mn = 10000
    for ins in instances:
        ...
        if val < mn: mn = val
        if val > mx: mx = val
    if mx-mn > bestRange: bestR = mx-mn, i
instances.sort(key = lambda a:a[bestR[1]])
for i in range(k):
    lst = instances[int(math.floor(iv*i)): int(math.floor(iv*(i+1)))]
    avg = [0, 0, 0, 0]
    for ins in lst:
