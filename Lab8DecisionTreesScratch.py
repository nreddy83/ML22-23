...
training_sets = []
for i in range(10): # different sample sets
    lst = []
...
# training_instances = [["low", "False", "False"], ["low", "True", "True"], ["mid", "False", "False"], ["mid", "True", "True"], ["high", "False", "True"], ["high", "True", "True"]]
# testing_instances = [["low", "False", "False"], ["low", "True", "True"]]
# attributes = ["GPA", "studied"]
# classes = ["True", "False"]
# dct = {"studied": ["True", "False"], "GPA": ["low", "mid", "high"]}
...
def makeTree(decisionTree, lst_instances, tree, head, dctToAdd):
    ...
    # determining best attribute to split this specific branch into
        ...
        # building the branch
        tree.append((bestAbt, attributes[bestAbt]))
        lst = []
        newdct = {}
        newdct[head] = []
...
decisionTree = makeTree(decisionTree, training_instances, tree, "root", {})   
print(decisionTree)

for training_set in training_sets:
    tree = []
    for i in range(math.sqrt(len(attributes))):
        r = random.randint(0, len(attributes))
...
for instance in training_instances:
    ...
    if val not in decisionTree: continue
    while b == False and "res" not in decisionTree[val]:
        ctr += 1
        v = instance[tree[ctr][0]]
        val = str(tree[ctr][1]) + str(v)
        if val not in decisionTree: b = True
    if b == True: continue
    r = decisionTree[val]
    res = classes[int(r[-1])]
    if res == instance[-1]: totSc += 1
